...
class SubModuleWindow:
  ...
  __fout = tempfile.NamedTemporaryFile(suffix=".png")
  wincfg = {}
  __counter = 0
  __imgs = []
  __algo = []
  ...
  def __fft(self,x, parents=None):
    ...
    if parents == None:
      ...
      self.__wait_for_update_call(position=[0])
    ...
    if N > 1:
      ...
      self.__wait_for_update_call(position=[7,8])
      even = x[::2]
      odd = x[1::2]

      # Perform FFT on even elements (each call cuts the sequence down by half)
      self.__wait_for_update_call(position=[10])
      x = self.__fft(even, parents = even)

      # Perform FFT on odd elements (each call cuts the sequence down by half)
      self.__wait_for_update_call(position=[11])
      x += self.__fft(odd, parents = odd)

      # Get your twiddle factor for the final calculations
      self.__wait_for_update_call(position=[13])
      T = np.exp(-2*np.pi*1j/N)

      self.__wait_for_update_call(position=[14])
      for k in range(N//2):
      ...
      self.__wait_for_update_call(position=[22])
      ...
      self.__wait_for_update_call(position=[4])
